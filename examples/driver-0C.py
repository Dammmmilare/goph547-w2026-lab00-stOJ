...
def Part_C():
    ...
    fig, (ay3, ay4) = plt.subplots(1, 2)
    ay3.plot(grey_scale[:, 200], color='black')
    ay3.set_title('Grey Scale Image Column 200')
    ay4.plot(img[:, 200, 0], color='red', label='Red Channel')
    ay4.plot(img[:, 200, 1], color='green', label='Green Channel')
    ay4.plot(img[:, 200, 2], color='blue', label='Blue Channel')
    ay4.set_title('RGB Image Column 200') 
    ay4.legend()
    plt.tight_layout()
